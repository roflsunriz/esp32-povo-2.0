...
`python scripts/generate-font.py` は `include/ui-text.h` の日本語文字と印字可能なASCII 95文字を抽出し、`include/japanese-font.h` を生成します。日本語は16×16、数字・英字は8×16で高さと書体を統一します。元ファイルのハッシュと不足字形を検査します。生成ヘッダーは手編集しません。追加の描画ライブラリとその保守負担を避け、小さな固定字形をTFTへ描画するために採用しました。
