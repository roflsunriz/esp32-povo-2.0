...
int main(int argc, char** argv) {
  ...
  try {
    ...
    const char* messages[] = {text::title, text::unknown, text::pending, text::stale,
      ...
      text::tabStatus, text::tabSleep, text::sleepTitle,
      text::sleepCombined, text::sleepAlwaysOn, text::sleepMinutesLabel,
      text::sleepHoursLabel, text::sleepPollLabel, text::sleepAlwaysNote,
      text::rotateHint, text::fetchingText,
      text::displayMemoryError};
    for (const char* message : messages) { checkLine(message); drawDisplay(nullptr, 0, message, 0, false); }
    ...
    snprintf(buffer, sizeof(buffer), text::nextPoll, 4ULL, 12ULL); checkLine(buffer);
    Status s;
    ...
    for (int source = 0; source < 2; ++source) {
      ...
      for (uint64_t elapsed : {0ULL, 900000ULL, 500000000ULL}) {
        drawDisplay(&s, elapsed, text::connection, 0, false);
    ...
    for (uint32_t seconds : {0U, 15U, 30U, 60U, 120U, 300U, 600U, 1800U, 3600U,
                             7200U, 7800U, 43200U, 86400U, 89940U}) {
      if (!setSleepTimeout(seconds)) throw std::runtime_error("known timeout rejected");
    ...
    if (setSleepTimeout(89941U)) throw std::runtime_error("overlong timeout accepted");
    if (!setPollIntervalSec(300U)) throw std::runtime_error("default poll rejected");
    if (!setPollIntervalSec(60U) || !setPollIntervalSec(600U))
      throw std::runtime_error("poll slider ends rejected");
    if (setPollIntervalSec(61U) || setPollIntervalSec(601U))
      throw std::runtime_error("off-step poll accepted");
    setPollIntervalSec(300U);
    if (!setSleepTimeout(povo::display::sleepTimeoutFromParts(10U, 2U)))
      throw std::runtime_error("slider timeout rejected");
    setDisplayPage(povo::display::Page::Sleep);
    drawDisplay(&s, 120000, nullptr, 0, false);
    ...
    s.expiryAtMs = 0; drawDisplay(&s, 0, nullptr, 0, false);
